...
# Freeze all the layers in the VGG19 model
...
results_dir = 'training_results_vgg19_6_classes_3'
os.makedirs(results_dir, exist_ok=True)

checkpoint_path = os.path.join(results_dir, 'best_model_vgg19.keras')
...
history_df.to_csv(os.path.join(results_dir, 'training_history_vgg19.csv'))
...
plt.savefig(os.path.join(results_dir, 'confusion_matrix_train_vgg19.png'))
plt.close()
...
with open(os.path.join(results_dir, 'classification_report_train_vgg19.txt'), 'w') as f:
...
plt.savefig(os.path.join(results_dir, 'confusion_matrix_val_vgg19.png'))
plt.close()

report_val = classification_report(y_val_classes, Y_val_pred_classes, target_names=list(val_generator.class_indices.keys()))
with open(os.path.join(results_dir, 'classification_report_val_vgg19.txt'), 'w') as f:
    f.write(report_val)
...
plt.savefig(os.path.join(results_dir, 'confusion_matrix_vgg19.png'))
plt.close()

report = classification_report(y_test_classes, Y_pred_classes, target_names=list(test_generator.class_indices.keys()))
with open(os.path.join(results_dir, 'classification_report_vgg19.txt'), 'w') as f:
    f.write(report)

print("Model training and evaluation complete. Results saved in 'training_results_vgg19_6_classes_1' directory.")
